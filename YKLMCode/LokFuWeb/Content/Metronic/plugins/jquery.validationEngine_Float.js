(function ($) {
    var methods = {
        init: function (options) { var form = this; if (!form.data('jqv') || form.data('jqv') == null) { methods._saveOptions(form, options); $(".formError").live("click", function () { $(this).fadeOut(150, function () { $(this).remove(); }); }); } }, attach: function (userOptions) {
            ...
            if (!rule) { alert("自定义规则未找到 " + customRule); return; }
            if (rule["regex"]) {
                var ex = rule.regex; if (!ex) { alert("验证正则错误 " + customRule); return; }
                var pattern = new RegExp(ex); if (!pattern.test(field.val()))
                    return options.allrules[customRule].alertText;
            } else if (rule["func"]) {
                fn = rule["func"];
                if (typeof (fn) !== "function") {
                    alert("验证函数错误" + customRule);
                    return;
                }
                if (!fn(field, rules, i, options))
                    return options.allrules[customRule].alertText;
            } else {
                alert("自定义规则错误" + customRule);
                return;
            }
                        ...
                        var errorFieldId = json[0]; var errorField = $($("#" + errorFieldId)[0]); if (errorField.length == 1) {
                            var status = json[1]; var msg = json[2]; if (!status) {
                                options.ajaxValidCache[errorFieldId] = false; options.isError = true; if (msg) {
                                    ...
                                    msg = rule.alertText; methods._showPrompt(errorField, msg, "", true, options);
                            } else {
            ...
            var prompt = $('<div>'); prompt.addClass(methods._getClassName(field.attr("id")) + "formError"); if (field.is(":input")) prompt.addClass("parentForm" + methods._getClassName(field.parents('form').attr("id"))); prompt.addClass("formError"); switch (type) { case "pass": prompt.addClass("greenPopup"); break; case "load": prompt.addClass("blackPopup"); }
            if (ajaxed)
            ...
            var className = "." + methods._getClassName(field.attr("id")) + "formError"; var match = $(className)[0]; if (match)
                return $(match);
    ...
    $.validationEngineLanguage = {
        newLang: function () {
            $.validationEngineLanguage.allRules = {
                ...
                "float1": {
                    "regex": /^(0|[1-9][0-9]*)(.[0-9]{1})?$/,
                    "alertText": "* 请输入整数或1位小数 "
                },
                "float": {
                    "regex": /^[\-\+]?(([0-9]+)([\.,]([0-9]+))?|([\.,]([0-9]+))?)$/,
                    "alertText": "* 请输入浮点数 "
                ...
                "check": {
                    ...
                    "alertTextOk": "* 检查结束，可以使用。",
                    "alertTextLoad": "* 检查重复数据中, 请稍后...",
                    "alertText": "* 数据已存在，不能使用。"
                },
                "idcard": {
