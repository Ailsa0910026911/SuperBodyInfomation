function Phone() {
    ...
    {
        var MoveClass = "draglist";
        var $UI = this.UI;
        $.each(Home, function (i, n) {
            var HomeMoveClass = "";
            if(n.IsLock != true)
            {
                HomeMoveClass = MoveClass;
            }
            $UI.featlist.append(
                '<li class="' + HomeMoveClass + ' native" id="' + n.Id + '" data-DisplaySite="1">\
        ...
        $.each(More, function (i, n) {
            var MoreMoveClass = "";
            if (n.IsLock != true) {
                MoreMoveClass = MoveClass;
            }
            $UI.movelist.append(
                '<li class="' + MoreMoveClass + ' native" id="' + n.Id + '" data-DisplaySite="3">\
                    <a href="javascript:void(0);">\
                    <img src="' + imgpath + n.PictureUrl + '"/>\
                    <p>' + n.Name + '</p>\
                    </a>\
        ...
        $.each(Boot, function (i, n) {
            var BootMoveClass = "";
            if (n.IsLock != true) {
                BootMoveClass = MoveClass;
            }
            $UI.bottlist.append(
                '<li class="' + BootMoveClass + ' bottomfeat" id="' + n.Id + '" data-DisplaySite="2">\
                    <a href="javascript:void(0);">\
    ...
    this.FloatIconMenu = function FloatIconMenu(obj) {
        ...
        obj.find("li.native").on("mouseover", function (e) {//浮动菜单
            ...
            $Html.lilasy.html("");
            var id = $(this).attr("id");
            var DisplaySite = $(this).attr("data-displaysite");
            //按区域确定数据源
            var datas = null;
            if (DisplaySite == 1) {
                datas = Home;
            ...
            var data = FindById(datas, id);
            
            if (data.IsLock == true) {
                $Html.lilasy.append($Html.lock);
            }
            else {
...
function Html() {
    ...
    this.lock = '<img src="/Content/HaoFu/image/lock_32.png" atl="锁定" />';
    this.bootAdd = '';
    if (IsAdd) {
