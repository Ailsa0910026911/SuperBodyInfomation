...
namespace GoodPayJobs
{
    ...
    /// 执行任务/每10分钟执行一次
    /// </summary>
    public class JobJPayQuery : IJob
    {
        public static bool IsRun = false;
        public void Execute(IJobExecutionContext context)
        {
            string JobName = "JPayQuery";
            ...
            if (CanRun == "true")
            {
                if (!IsRun)
                {
                    ...
                    try
                    {
                        ...
                        IList<JobItem> JobItemList = Entity.JobItem.Where(n => n.State == 2 && n.RunedTime > STime && n.RunedTime <= ETime && n.RunType == 1 && n.RunState == 2).ToList();//获取10分钟前的未明状态订单
