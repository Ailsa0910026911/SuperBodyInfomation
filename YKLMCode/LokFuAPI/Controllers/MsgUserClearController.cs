...
{
    public class MsgUserClearController : InitController
    {
        public MsgUserClearController()
        {
            if (!InitState)
            {
        ...
        public void Post()
        {
            ...
            IList<MsgUser> MsgUserList = Entity.MsgUser.Where(n => n.UId == baseUsers.Id).ToList();
            //删除自己的
            string DeleteIds = "0";
            ...
            MsgUserList = Entity.MsgUser.Where(n => n.UId == 0 && !n.DeleteUsers.Contains(uid)).ToList();
            //全员标识已删除
